...
import { BOARD_PEG_COUNT, PATTERN_PRESETS, type PatternParams, type PatternPreset } from '../lib/patterns'
  ...
  return (
    <div className="flex shrink-0 flex-col gap-4 rounded-[1px] border border-line bg-bg-1 p-5">
            ...
            <div key={preset.id} className="flex flex-col">
              ...
              {isActive && (
                <div className="flex flex-col gap-3 border border-t-0 border-red bg-bg-2/50 p-3">
                  ...
                  {blocker && <span className="font-mono text-[0.55rem] leading-relaxed text-red">{blocker}</span>}
      ...
      <p className="font-mono text-[0.58rem] leading-relaxed text-text-2 opacity-70">
        ...
        board. Applying one leaves the peg count alone; while one is active the board's peg count and the
        thread colours drive it live — move either and the pattern redraws. Line weight doesn't apply here.
        Clear, or generate from an image, to hand the preview back to the solver.
      </p>
    </div>
  )
}
